...
public class PackageDBUtil {
    ...
    private static final String INSERT_PKG_SQL = "INSERT INTO packages (PkgName, Description, Price, Duration, PkgType) VALUES (?, ?, ?, ?, ?)";
    ...
    private static final String UPDATE_PKG_SQL = "UPDATE packages SET PkgName=?, Description=?, Price=?, Duration=?, PkgType=? WHERE ID=?";
    ...
    public void insertPackage(Package pkg) throws SQLException {
        try (Connection conn = DBConnect.getConnection();
            ...
            preparedStatement.setString(5, pkg.getPkgType());
    ...
    public boolean updatePackage(Package pkg) throws SQLException {
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            preparedStatement.setInt(4, pkg.getDuration());
            preparedStatement.setString(5, pkg.getPkgType());
            preparedStatement.setInt(6, pkg.getId());
    ...
    public Package selectPackage(int id) {
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            while (rs.next()) {
                ...
                pkg = new Package(id, pkgName, description, price, duration, pkgType);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Package> selectAllPackages() {
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            while (rs.next()) {
                ...
                packages.add(new Package(id, pkgName, description, price, duration, pkgType));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
